    ...
    public spawn(cwd = process.cwd()) {
        const promise = new Promise((resolve, reject) => {
            ...
            call.on('exit', (code) => {
                ...
                LOG.info(`spawned:: ${this.cmd} code::${code}`);
                if (code === 0) {
                    resolve(0);
                } else {
                    reject({ code, stdout: this.stdoutstr });
                }
            });
...
export const shellcmds = async (cmds: string[], env?: Environment): Promise<string[]> => {
    const retvals = [];
    ...
    for (const c of cmds) {
        const cmd = new Cmd(c, cmdEnv as Environment);
        await cmd.spawn();
...
export const shellcmd = async (cmd: string, env?: Environment): Promise<string[]> => {
    const cmdEnv = process.env;
    if (env) {
        Object.assign(cmdEnv, env);
    }

    const c = new Cmd(cmd, cmdEnv as Environment);
    await c.spawn();
    return c.stdoutstr;
};
